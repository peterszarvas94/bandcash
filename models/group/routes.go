...
func RegisterRoutes(e *echo.Echo) *Group {
	...
	usersRoutes.GET("/pending", grp.PaymentsPage)
	...
	adminUsersRoutes.PUT("/pending/events/:id/toggle-paid", grp.TogglePaymentEventPaid)
	adminUsersRoutes.POST("/pending/events/:id/paid_at", grp.UpdatePaymentEventPaidAt)
	adminUsersRoutes.PUT("/pending/participants/:eventId/:memberId/toggle-paid", grp.TogglePaymentParticipantPaid)
	adminUsersRoutes.POST("/pending/participants/:eventId/:memberId/paid_at", grp.UpdatePaymentParticipantPaidAt)
	adminUsersRoutes.PUT("/pending/expenses/:id/toggle-paid", grp.TogglePaymentExpensePaid)
	adminUsersRoutes.POST("/pending/expenses/:id/paid_at", grp.UpdatePaymentExpensePaidAt)
